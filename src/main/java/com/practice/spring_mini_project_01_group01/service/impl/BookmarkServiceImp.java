...
public class BookmarkServiceImp implements BookmarkService {
    ...
    public CustomResponse<BookmarkResponse> addBookmarkByArticleId(Long articleId) {
        ...
        bookmarkRepository.findByArticle_ArticleIdAndUser_Id(articleId, user.getId())
                .ifPresent(b -> {
                    throw new BadRequestException("Already bookmarked");
                });

        Bookmark bookmark = bookmarkRepository.save(
                Bookmark.builder().article(article).user(user).build()
        );
    ...
    @Override
    public CustomResponse<List<BookmarkResponse>> getAllBookmarksByCurrentUser(int page, int size, Sort.Direction direction) {
        User user = authUtil.getCurrentUser();
        ...
        List<BookmarkResponse> payload = bookmarkPage.stream()
